...
function print_ticketCard_form($ticketId = 0, $socId = 0, $action = '')
{
	...
	$out .= '<form role="form" autocomplete="off" class="form" method="post"  action="'.$context->getRootUrl('ticket_card').'" >';
		...
		$out .= '<div class="form-ticket-message-container" >';
			...
			//Type de tickets - menu dÈroulant
			...
			//Extrafield concernantles tickets ‡ rÈcupÈrer avec menu dÈroulant: vÈhicule, Atelier, DatÈ dÈbut, date fin
			$extrafields = new ExtraFields($db);
			$tickets = $extrafields->fetch_name_optionals_label('ticket');
			foreach ($tickets as $key=>$label){				
				$out .= '<div class="form-group">';	
...
function print_ticketCard_comment_form($ticket, $action = '', $timelineIntegration = 1)
{
	...
	$status = (int)$ticket->fk_statut; // TODO : v√©rifier sur les nouvelles version si le nom du champ na pas chang√©
	...
	if($dropDown){
		// j'ai pr√©par√© un drop down pour plus tard au niveau interface utilisateur
