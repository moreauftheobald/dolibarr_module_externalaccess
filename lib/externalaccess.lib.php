...
function stdFormHelper($name = '', $label = '', $value = '', $mode = 'edit', $htmlentities = true, $param = array()) {
	...
	if (! empty ( $param ['required'] ) && $mode != 'readonly') {
		print '*';
	}
	print '</label>';
 ...
 *
 * @param array $a
 *        	PDF lines array fields configs
 * @param array $b
 *        	PDF lines array fields configs
...
function load_last_main_doc(&$object) {
	...
	if (empty ( $object->last_main_doc )) {
		...
		if (is_readable ( DOL_DATA_ROOT . '/' . $last_main_doc ) && is_file ( DOL_DATA_ROOT . '/' . $last_main_doc ))
			$object->last_main_doc = $last_main_doc;
	}
}
